	...
	*/
	function setData() {
		...
		for (var i = 0; i < 6; i++) {
			for (var j = 0; j < 7; j++) {
				...
				if (check == true && position <= numberDay) {
					...
					document.getElementById("item" + (position + day)).style.background = "#66FF33";
		...
		for (var i = positionStart; i > 0; i--) {
			...
			document.getElementById("item" + (i)).style.background = "#9C9C9C";
		...
		for (var j = 1; j <= (42 - positionStop); j++) {
			...
			document.getElementById("item" + (j + positionStop)).style.background = "#9C9C9C";
